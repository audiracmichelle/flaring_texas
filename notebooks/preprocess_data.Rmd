...
#   rename(pop = estimate)
# 
# write_rds(tract_acs, "../data/preprocessed/tract_acs.rds")
```

```{r, warning=F, message=F}
#### get tracts population
tract_acs <- read_rds("../data/preprocessed/tract_acs.rds")
...
* Exact location of flares

```{r}
viirs <- read.csv("../data/input/viirs.csv")

viirs %<>% 
  mutate(
    time = as.POSIXct(Date_Proc), #or Date_Mscan?
    date = as.Date(time), 
    start_hour = hour(time)
  ) %>% 
  rename(
    Latitude = Lat_GMTCO, #or Lat_DNB
    Longitude = Lon_GMTCO
  )

viirs_sf <- viirs %>% 
  st_as_sf(coords = c("Longitude", "Latitude"), crs = "28992") %>% 
  st_set_crs("NAD83") %>% 
  st_transform(crs_)

ggplot() + 
  geom_sf(data = state_sf) + 
  geom_sf(data = viirs_sf, size = 0.5)
```

```{r}
#### join tracts and viirs 
index <- viirs_sf %>% 
  st_intersects(tracts_sf) %>% 
  as.numeric()

viirs$GEOID <- tracts_sf$GEOID[index]
```

* prepare disperseR input
...
```{r}
#### disperseR input viirs
ID <- unique(viirs[,c("Latitude", "Longitude")])
ID$ID <- 1:nrow(ID)

viirs %<>%
  left_join(ID)

disperser_input_viirs <- viirs %>% 
  select(ID, GEOID, date, start_hour, Latitude, Longitude)

disperser_input_viirs %<>% 
  rename(start_day = date) %>% 
  mutate(uID = ID, 
         Height = 20, 
         year = year(start_day), 
         month = month(start_day),
...
write_rds(viirs, "../data/preprocessed/viirs.rds")
write_rds(tracts_sf, "../data/preprocessed/tracts_sf.rds")
write_rds(disperser_input, "../data/jobs_input/disperser_input.rds")
write_rds(disperser_input_viirs, "../data/jobs_input/disperser_input_viirs.rds")
```
