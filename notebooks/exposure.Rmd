...
```{r, include=FALSE}
fracking_dispersion <- tracts_sf %>% 
  st_drop_geometry() %>% 
  mutate(exposed_tracts_flares = (!is.na(tot_flares)), 
         exposed_pop_flares = pop * exposed_tracts_flares, 
         exposed_tracts_particles = (!is.na(tot_particles)),
         exposed_pop_particles = pop * exposed_tracts_flares) %>% 
  ...
  summarise(tot_flares = sum(tot_flares, na.rm = T), 
            ...
            n = n(), 
            exposed_tracts_flares = sum(exposed_tracts_flares), 
            exposed_tracts_particles = sum(exposed_tracts_particles)) %>% 
  mutate(exposed_tracts = exposed_tracts_flares + exposed_tracts_particles) %>% 
  filter(exposed_tracts > 0) %>% 
  arrange(desc(pop_cbsa))

#### write in googlesheets
# ss <- as_sheets_id("https://docs.google.com/spreadsheets/d/1KAfn6-NqCodkZ62nS4Vo92JnsNLudaRDuOncBw-AQmI/edit?usp=sharing")
# sheet_write(fracking_dispersion, ss) # <- run if updating the spreadsheet
# gs4_browse(ss) # <- to browse spreadsheet 
```
