...
```{r}
tracts_sf <- st_read("../data/local/nhgis0026_shapefile_tl2016_us_tract_2016/US_tract_2016.shp")
tracts_sf <- tracts_sf %>% 
...
flares <- lapply(input$flares, function(x) floor(seq(6,17,12/x)))
input$start_hour = flares
input <- unnest(input, cols = c(start_hour))
head(input)
```

```{r}
write_rds(input, "../data/input.rds")
```
